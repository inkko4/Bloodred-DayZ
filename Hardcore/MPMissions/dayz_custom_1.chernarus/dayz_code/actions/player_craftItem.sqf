...
_config = (missionconfigFile >> _isClass >> _classname >> "ItemActions" >> "Crafting");
...
if(!r_drag_sqf and !r_player_unconscious and !_onLadder) then {
	...
	if (_classname == "ItemLog") then {
	_input = [["ItemLog","CfgMagazines",1]];
	_output = [["ItemPlank","CfgMagazines",2]];
	_required = ["ItemHatchet"];
	_failChance = 0;
	};
	if (_classname == "ItemPlank") then {
	_input = [["ItemPlank","CfgMagazines",1]];
	_output = [["PartWoodPile","CfgMagazines",2]];
	_required = ["ItemHatchet"];
	_failChance = 0;
	};
	_hasTools = true;
