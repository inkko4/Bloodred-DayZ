if (!isnil ("VehOffroad")) then {
	// if defined adds to existing variable
	if ((getplayeruidold player) in VehOffroad) then {
	...
	_veh = createVehicle ["LandRover_CZ_EP1", _pos,[],0,"CAN_COLLIDE"];
	_veh setPosATL _pos;
	_veh setDir _dir;
	...
	VehOffroad = [_id]+VehOffroad;
	publicVariable "VehOffroad";
