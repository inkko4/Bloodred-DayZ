if (!isnil ("VehHmmwv")) then {
	// if defined adds to existing variable
	if ((getplayeruidold player) in VehHmmwv) then {
	...
	_veh = createVehicle ["HMMWV_DZ", _pos,[],0,"CAN_COLLIDE"];
	_veh setPosATL _pos;
	_veh setDir _dir;
	...
	VehHmmwv = [_id]+VehHmmwv;
	publicVariable "VehHmmwv";
