...
if (_inVehicle && driver _vehicle == player) then {
	...
	if (s_player_inCarlockUnlock_crtl < 0 && _hasKeyinCar) then {
		...
		if(locked dayz_myLockedVehicle) then {
			_Unlock = dayz_myLockedVehicle addAction [format[localize "STR_EPOCH_ACTIONS_UNLOCK",_text], "fixes\unlocklockincar.sqf",[dayz_myLockedVehicle, false, (_temp_keys_names select (parseNumber _carID))], 2, true, true, "", ""];
			s_player_incarlockunlock set [count s_player_incarlockunlock,_Unlock];
			s_player_inCarlockUnlock_crtl = 1;
		} else {
			_lock = dayz_myLockedVehicle addAction [format[localize "STR_EPOCH_ACTIONS_LOCK",_text], "fixes\unlocklockincar.sqf",[dayz_myLockedVehicle, true, (_temp_keys_names select (parseNumber _carID))], 1, false, true, "", ""];
...
if (!isNull cursorTarget && !_inVehicle && !_isPZombie && (player distance cursorTarget < _allowedDistance) && _canDo) then {	//Has some kind of target
	...
	_hasArrestItem = "PartGeneric" in _itemsPlayer;
	_hasDoorDestroyer = "PMC_ied_v4" in _magazinesPlayer;
	...
	_isModularDoorLocked = _typeOfCursorTarget in DZE_DoorsLocked;
	...
	if(_isAlive) then {
		...
		// Destructible Door - Place bomb
		/*if(_isModularDoorLocked || _isModularDoor) then{
			if(_hasDoorDestroyer) then {
				if ((s_player_destroydoor < 0) && (player distance _cursorTarget < 4)) then {
					s_player_destroydoor = player addAction [("<t color=""#B40404"">" + ("Destroy Door") +"</t>"), "scripts\DoorBomb.sqf",_cursorTarget, 0, false, true, "", ""];
				}
			}else{
				player removeAction s_player_destroydoor;
				s_player_destroydoor = -1;
			}; 
		};*/
	...
	if (!locked _cursorTarget && {player distance _cursorTarget < 5} && {(_cursorTarget isKindOf "LandVehicle") || (_cursorTarget isKindOf "Air") || (_cursorTarget isKindOf "Ship")}) then {
		if (s_player_forceGetIn < 0) then {
			s_player_forceGetIn = player addaction ['<t color="#0074E8">' + "Get in Vehicle" + '</t>', "fixes\getin.sqf",_cursorTarget,100,false,true,"", ""];
		};
	} else {
		player removeAction s_player_forceGetIn;
		s_player_forceGetIn = -1;
	};
...
//--------------------------------------ARREST---------------------------------------------------------------- 
		   if (_hasArrestItem || (getPlayerUID player) in AdminList ) then {
...
/////////////////////////////////////////////////////////////////////////////////////////MANAGE DOOR END////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//Currency
		...
		if (s_bank_dialog < 0) then {
			if (_typeOfCursorTarget in ["VaultStorage","Laptop_EP1"]) then {
				s_bank_dialog = player addAction ["ATM", "ZSC\actions\bank_dialog2.sqf",_cursorTarget, 3, true, true, "", ""];
			} else {
				s_bank_dialog = player addAction ["Cash Storage", "ZSC\actions\bank_dialog.sqf",_cursorTarget, 3, true, true, "", ""];
			};
		};
	} else {
...
if ((speed player < 1) && player getVariable "HeroPerk" > 0) then {
	if (s_player_heroperk < 0) then {
		s_player_heroperk = player addAction [("<t color='#c30000'>")+("Hero Perk")+("</t>"), "scripts\HeroPerks\reward.sqf","",5,false,true,"",""];
	};
}else {
	player removeAction s_player_heroperk;
	s_player_heroperk = -1;
};
