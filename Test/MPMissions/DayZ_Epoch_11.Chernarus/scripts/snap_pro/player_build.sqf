...
if (_hasRequiredTools && _hasbuilditem) then {
	...
	if(!_cancel) then {
		...
		_door = ["Land_DZE_GarageWoodDoor","Land_DZE_LargeWoodDoor","Land_DZE_WoodDoor","Land_DZE_GarageWoodDoorLocked","Land_DZE_LargeWoodDoorLocked","Land_DZE_WoodDoorLocked","CinderWallDoorway_DZ","CinderWallDoor_DZ","CinderWallDoorLocked_DZ","CinderWallSmallDoorway_DZ","CinderWallDoorSmall_DZ","CinderWallDoorSmallLocked_DZ"];
		
		if (typeOf(_tmpbuilt) in _door) then {
			_tmpbuilt addEventHandler ["HandleDamage", {true}];
			_tmpbuilt enableSimulation true;
		};
		if (!(typeOf(_tmpbuilt) in _door)) then {
