...
if (!isDedicated) then {
	...
	[] spawn {
		while {1 == 1} do {
			sleep 600;
			systemchat "You have recieved a 5,000 coin paycheck.";
			_wealth = player getVariable["cashMoney",0];
			_newwealth = _wealth + 5000;
	...
	[] spawn {
		_bankvalue = profileNamespace getVariable "atmBank";
		if (isnil "_bankvalue") then {
			profileNamespace setVariable ["atmBank",0];
			player setVariable ["atmBank2",0,true];
		} else {
			player setVariable ["atmBank2",_bankvalue,true];
		};
		saveProfileNamespace;
	};
	//execVM "bankupdate.sqf";
	[] spawn {
