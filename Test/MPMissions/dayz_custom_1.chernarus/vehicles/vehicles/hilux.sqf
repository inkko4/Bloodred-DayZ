if (!isnil ("VehHilux")) then {
	// if defined adds to existing variable
	if ((getplayeruidold player) in VehHilux) then {
	...
	_veh = createVehicle ["hilux1_civil_3_open", _pos,[],0,"CAN_COLLIDE"];
	_veh setPosATL _pos;
	_veh setDir _dir;
	...
	VehHilux = [_id]+VehHilux;
	publicVariable "VehHilux";
