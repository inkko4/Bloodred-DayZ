...
//temp settings
dayz_DamageMultiplier = 0.5; //1 - 0 = Disabled, anything over 1 will multiply damage. Damage Multiplier for Zombies.
...
sleep 5;
	_loadoutstatus = profileNamespace getVariable 'Loadout_Togg';
	if (isnil '_loadoutstatus') then {
		profileNamespace setVariable ['Loadout_Togg',false];
	};
